classdef ARobotarium < handle
    ...
    properties (GetAccess = protected, SetAccess = protected)
        ...
        mat_file_path
        boundary_patch
%         boundary_points = {[-1.5, 1.5, 1.5, -1.5], [-1.5, -1.5, 1.8, 1.8]}; 
        boundary_points = {[-1.5, 1.6, 1.6, -1.5], [-1.5, -1.5, 1.9, 1.9]}; % Chendi: modified for IROS 2020
    end
    ...
    methods (Access = protected)
        ...
        function initialize_visualization(this)
            ...
            %plot(im)
            set(figPhi,'color','white'); % modified for IROS 2020
            % 'Position',[0 0 800 1080]
            % Set axis
            robotPlaneAxes = gca;
        ...
        function draw_robots(this)
            ...
            for ijk = 1:size(this.conn_matrix, 1)
                ...
                conn_handle_tmp{ijk}=line('XData', [this.poses(1, node1_idx), this.poses(1, node2_idx)], ...
                    'YData', [this.poses(2, node1_idx), this.poses(2, node2_idx)], ...
                    'Color', [1 .5 .5], 'LineStyle','-','LineWidth',1);
            end
...
%                 end
                if ~isempty(this.type_flag)
                    if this.type_flag(ii)==1 % added by Chendi, to change bot color due to type_flag
                        new_colors(1,:) = [1 0 0]; % secondary to red
                    elseif this.type_flag(ii)==0
                        new_colors(1,:) = [0 0 1]; % primary to blue
                    elseif this.type_flag(ii)==2
                        new_colors(1,:) = [0 1 0];
                    elseif this.type_flag(ii)==3
                        new_colors(1,:) = [0.5 0.3 0.5]; 
                    elseif this.type_flag(ii)==4
                        new_colors(1,:) = [0 0 0]; 
                    end
                    set(this.robot_handle{ii},'Vertices', transformed(:, 1:2),'FaceVertexCData', new_colors);
                else
