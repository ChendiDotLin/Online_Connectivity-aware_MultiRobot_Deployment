...
%Modified by Wenhao Luo (whluo12@gmail.com)
%A general framework for formation control - set up a rectangle perimeter
%with size of S x L
...
n = 2; % defining how many segments along each side, namely (n+1) bots each side including the vertices
N = 4*n; %number of robots is 4(n+1)-4=4n due to 1-extra counting of each vertex as each of them belongs to 2 sides 
...
% get desired laplacian matrix and inter-agent distance matrix to get a
% rectangle formation

d1=0.3;
d2=0.3;

[L,weights] = GetFormationMatrix(N,d1,d2);

% may need offset pos to robot 1 so that the formation biased towards a
% determined pos
...
for t = 0:iterations
    ...
    dx = si_barrier_cert(dx, x);  % debug only, temporarily get rid of
%     the barriers
