function [assigned,w] = mgm_no_coalition(cap,w,value,assigned,poses,targets)
...
%             end
        ...
        for j = 1:nt+1
            ...
            improved(j) = sum(min(cap(:,k),max(zeros(num_cap,1),w(:,j))))*value(j) - loss + alpha/(1+dist); % how much a robot can fulfill
            end
