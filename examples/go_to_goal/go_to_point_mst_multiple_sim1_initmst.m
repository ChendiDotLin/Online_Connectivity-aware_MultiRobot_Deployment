...
min_bot_dist_hist_initmst = zeros(1500,1);
conn_bot_hist_initmst = zeros(1500,1);
pertub_bot_hist_initmst = zeros(1500,1);
perf_bot_hist_initmst = zeros(1500,1);
...
while (timer_count<1501)%(~init_checker(x, initial_conditions))
    ...
    min_bot_dist_hist_initmst(timer_count) = min_bot_dist_tmp;
...
%   dxi = controller(x(1:2, :), initial_conditions(1:2, :));
   ...
   perf_bot_tmp = sum(sqrt(sum((x(1:2,:)-des_pos).^2, 1)))/N;
    ...
    perf_bot_hist_initmst(timer_count) = perf_bot_tmp;
...
%     dxi = controller(x(1:2, :), current_pos);
    ...
    conn_bot_hist_initmst(timer_count) = eig_tmp;
    ...
    pertub_bot_hist_initmst(timer_count) = pertub_bot_tmp;
