...
controller = create_si_con_controller();
...
%   dxi = controller(x(1:2, :), initial_conditions(1:2, :));
    if timer_count>timer_switch + 1e4
   ...
   % implement behavior controller (inputs: robot id, adj matrix, ctrl_vec, varargin (control parameters);  output: ctrl_vec)
    ...
    dxi = zeros(2,N);
    dxi0 = controller(lap_matrix0, x(1:2, member_idx0), current_pos(:,member_idx0));
    dxi1 = controller(lap_matrix1, x(1:2, member_idx1), current_pos(:,member_idx1));
    dxi2 = controller(lap_matrix2, x(1:2, member_idx2), current_pos(:,member_idx2));
    
    dxi(:,member_idx0) = dxi0;
    dxi(:,member_idx1) = dxi1;
    dxi(:,member_idx2) = dxi2;
%     
