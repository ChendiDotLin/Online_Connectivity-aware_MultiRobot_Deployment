...
min_bot_dist_hist_initcon = zeros(1500,1);
conn_bot_hist_initcon = zeros(1500,1);
pertub_bot_hist_initcon = zeros(1500,1);
perf_bot_hist_initcon = zeros(1500,1);

% controller = create_si_rendzvous_controller();
r.set_adj_flag(false); % adj_flag, when debug to plot sim1, turn it to true
while (timer_count<1501)%(~init_checker(x, initial_conditions))
    ...
    min_bot_dist_hist_initcon(timer_count) = min_bot_dist_tmp;
...
%   dxi = controller(x(1:2, :), initial_conditions(1:2, :));
   ...
   perf_bot_tmp = sum(sqrt(sum((x(1:2,:)-des_pos).^2, 1)))/N;
    ...
    perf_bot_hist_initcon(timer_count) = perf_bot_tmp;
...
if timer_count==1
    ...
    adj_tmp0 = adj_tmp;
    adj_matrix0 = adj2matrix(adj_tmp0);
     r.set_conn(adj_matrix0);
else
    [~, ~, ~, adj_tmp] = GetGraphWeight(x, 1.2*R2, dxi, ctrl_flag);
end
...
r.set_adj_matrix(adj_matrix);
    ...
    conn_bot_hist_initcon(timer_count) = eig_tmp;
    
    si_barrier_certificate = create_con_barrier_certificate('SafetyRadius', 0.06,'ConnRadius', sqrt(R2),'conn_matrix',adj_matrix);
    ...
    pertub_bot_hist_initcon(timer_count) = pertub_bot_tmp;
