% test code for rendzvous behaviour without collision avoidance
...
pri_pos = [0;0]; % primary rendzvous target location
sec_pos = [-0.6;0.8]; % secondary rendzvous target location
sec_pos0 = sec_pos;
timer_switch_sec = 0;%200; % time steps for switching to secondary
timer_switch_pri = 1000; % time steps for switching back to primary controller
...
figure(99);
handle_pri = text(pri_pos(1),pri_pos(2),'primary target');
handle_exp = text(-0.5,-1.3,'swarm (blue) move to primary target');
n_step = 100; %
% test_pos = rand(2,N);
handle_sec = [];
while timer_count < 1200 %(~init_checker(x, initial_conditions))

    x = r.get_poses();
    
    sec_pos = sec_pos0 + 0.1*sin(timer_count*pi/2/n_step);
    target_pos(:,ctrl_flag>0) = repmat(sec_pos,[1 numel(find(ctrl_flag>0))]); % assign secondary controller
...
%     conn_matrix_tmp = GetConnMatrix(x, R2_off*R2); % simply test whether the connectivity constraint is violated
    ...
    if (timer_count>=timer_switch_sec)&&(timer_count<timer_switch_pri)
        delete(handle_sec);
...
%     current_pos = test_pos;% test debug only
