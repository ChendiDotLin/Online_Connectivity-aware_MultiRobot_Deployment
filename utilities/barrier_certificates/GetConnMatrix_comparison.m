%% Get Connectivity Matrix from the Connected Graph as compared to the MST solution
...
function conn_matrix = GetConnMatrix_comparison(xx, R2)
N = size(xx,2);
...
if numel(unique(conncomp(G_graph)))~=1   %~isempty(find(all(logical(adj)==0,2), 1))
    error('initial robotic swarm not connected!');
end

% T_tree = minspantree(G_graph,'Type','tree');
conn_matrix = table2array(G_graph.Edges);
