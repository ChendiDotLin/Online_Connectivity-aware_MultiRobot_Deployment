...
%   1, 'YVelocityGain', 1);
%% Implementation
function [si_position_controller] = create_si_line_controller(varargin)
...
%         theta_vec = 1/N*2*pi*[1:N];
        ...
        x_poses = [1:N]/N*line_length-line_length/2;%radius_circle*cos(theta_vec);
        y_poses = 0*x_poses;% y = ax  % radius_circle*sin(theta_vec);
        poses = [x_poses;y_poses];
        
        % lets run some greedy role assignment to re-order formation
        % parameters in poses.
        ...
        for ijk = 1:N
            cur_bot = states(:,ijk);
            dist_list = pdist2(cur_bot',test_poses');
            [~,ass_idx] = min(dist_list);
            new_poses(:,ijk) = poses(:,ass_idx);
            test_poses(:,ass_idx)  = [Inf; Inf];
